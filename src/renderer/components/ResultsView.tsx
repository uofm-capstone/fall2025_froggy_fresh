...
export default function ResultsView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div>
      ...
      {/* Unified table layout with increased column spacing */}
      <div className="apple-card w-full p-0 overflow-hidden">
        <div className="grid grid-cols-12 gap-0">
          {/* Header row - spans full width */}
          <div className="col-span-12 grid grid-cols-12 border-b border-[var(--apple-border)]">
            {/* Runs header - takes up 2 columns */}
            <div className="col-span-2 p-4 font-medium text-[var(--apple-text)] bg-[#f0f0f0] dark:bg-[#2c2c2e] border-r border-[var(--apple-border)]">
              Runs
            </div>
                ...
                {runs.length > 0 ? (
                  runs.map((run, index) => (
                    <button
                      ...
                      className={`w-full text-left p-4 text-[var(--apple-text)] hover:bg-[#f0f0f0] dark:hover:bg-[#2c2c2e] transition-colors border-b border-[var(--apple-border)] ${
                ...
                {selectedRun && selectedRun.files.length > 0 ? (
                  selectedRun.files.map((file, idx) => (
                    <div
                      ...
                      className={`grid grid-cols-12 border-b border-[var(--apple-border)] text-[var(--apple-text)] hover:bg-[#f9f9f9] dark:hover:bg-[#2c2c2e]/50 cursor-pointer ${
            ...
            <div className="col-span-3">
              <div className="p-4 flex flex-col h-full">
                ...
                <div className="flex gap-4 mt-auto">
                  <button className="flex-1 bg-[var(--apple-green)] hover:opacity-90 text-white text-xl py-3 rounded-xl border-0 transition-opacity">
                    FROG
                  </button>
                  <button className="flex-1 bg-[var(--apple-red)] hover:opacity-90 text-white text-xl py-3 rounded-xl border-0 transition-opacity">
                    NOT FROG
                  </button>
                </div>
              </div>
