...
interface ImageResultData {
  ...
  override: boolean;
  camera: number;
  originalClassification?: "FROG" | "NOT FROG";
}
...
export default function ResultsView({ onBack }: { onBack: () => void }) {
  ...
  const selectRunEntry = async (entry: RunResultsEntry) => {
    if (selectedRun?.filePath === entry.filePath) {
      ...
      ipcRenderer.invoke("get-run-data", entry.filePath).then((runData: SelectedRunData | null) => {
        if (runData !== null) {
          // Initialize originalClassification if missing
          runData.results = runData.results.map((img) => ({
            ...img,
            originalClassification: img.originalClassification || img.classification,
          }));
  ...
  // Modified override function — only flips between FROG and NOT FROG,
  // and resets override to false if toggled back to original classification
  const overrideImageClassification = async () => {
    if (!selectedImage || !selectedRun) return;

    const currentLabel = selectedImage.image.classification;
    const newLabel = currentLabel === "FROG" ? "NOT FROG" : "FROG";

    const originalLabel = selectedImage.image.originalClassification || selectedImage.image.classification;

    const updatedImage = {
      ...selectedImage.image,
      classification: newLabel,
      override: newLabel !== originalLabel, // Only mark overridden if it differs from original
      originalClassification: originalLabel,
    };
          ...
          <div className="grid grid-cols-[3fr,1fr,1fr,1fr,1fr,0.65fr] gap-0 w-full text-sm bg-[var(--apple-body-bg)]">
            ...
            <div className="p-2 pl-3 font-medium text-[var(--apple-text)] bg-[var(--apple-body-bg)] border-r border-[var(--apple-border)]">
              {selectedRunResultsEntry ? (
                <div>
                  <div>
                    {selectedRunResultsEntry.date} {selectedRunResultsEntry.time}
                  </div>
            ...
            <div className="p-2 text-center font-medium text-[var(--apple-text)] bg-[var(--apple-body-bg)] border-r border-[var(--apple-border)] flex items-center justify-center">
              {selectedRun ? `${selectedRun.results.length} IMAGES` : "# IMAGES"}
            </div>
            <div className="p-2 text-center font-medium text-[var(--apple-text)] bg-[var(--apple-body-bg)] border-r border-[var(--apple-border)] flex items-center justify-center">
              {selectedRun ? `${selectedRun.frogs} FROG` : "# FROG"}
            </div>
            <div className="p-2 text-center font-medium text-[var(--apple-text)] bg-[var(--apple-body-bg)] border-r border-[var(--apple-border)] flex items-center justify-center">
              {selectedRun ? `${selectedRun.notFrogs} NOT FROG` : "# NOT FROG"}
            </div>
            <div className="p-2 text-center font-medium text-[var(--apple-text)] bg-[var(--apple-body-bg)] border-r border-[var(--apple-border)] flex items-center justify-center">
              {selectedRun ? (
                <div>
                ...
                <div>
                  <div>ACCURACY</div>
                  <div className="text-xs text-[var(--apple-subtle-text)]">(not overridden/total)</div>
                </div>
              )}
            </div>
            ...
            >
              <span
                className={`text-sm font-semibold ${selectedRun ? "text-white" : "text-gray-200"}`}
              >
                Export
              </span>
            </div>
      ...
      <div className="apple-card w-full p-0 overflow-hidden">
        <div className="grid grid-cols-11 gap-0">
          ...
          <div className="col-span-11 grid grid-cols-11 border-b border-[var(--apple-border)]">
            <div
              className="col-span-2 p-4 font-medium text-[var(--apple-text)]
            ...
            <div
              className="col-span-5 grid grid-cols-10 p-0 bg-[#f0f0f0]
            ...
            <div
              className="col-span-4 p-4 bg-[#f0f0f0] dark:bg-[#2c2c2e]
          ...
          <div className="col-span-11 grid grid-cols-11">
                  ...
                  runsList.map((run, index) => (
                    <button
                      ...
                      className={`w-full text-left p-4 text-[var(--apple-text)]
                                 ...
                                 ${
                                   selectedRun && selectedRun.filePath === run.filePath
                    ...
                    <div
                      ...
                      className={`grid grid-cols-10 border-b border-[var(--apple-border)]
                                 ...
                                 dark:hover:bg-[#2c2c2e]/50 cursor-pointer ${
                                   selectedImage &&
                                   selectedImage.image.imagePath === file.imagePath
            ...
            <div className="col-span-4">
              <div className="p-4 flex flex-col h-full">
                <div
                  className="relative w-full h-72 bg-[#1c1c1e] rounded-lg
                  ...
                  {selectedImage ? (
                    <>
                      {selectedImage.loading === false ? (
                        <img
                          src={`data:image/jpeg;base64,${selectedImage.image.rawData}`}
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <p className="text-white/50">
                            {selectedImage.image.name} (loading)
                          </p>
                        </div>
                      )}
                      <div
                ...
                <div className="flex gap-4 mt-4">
                  <button
                    className={`flex-1 text-white text-xl py-3 rounded-xl border-0 transition-opacity ${
                      selectedImage?.image.classification === "FROG"
                        ? "bg-[var(--apple-red)]"
                        : "bg-[var(--apple-green)]"
                    }`}
                    onClick={overrideImageClassification}
